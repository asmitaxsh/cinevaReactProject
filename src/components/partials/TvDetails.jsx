...
const handleShare = () => {
  if (navigator.share) {
    navigator.share({
      title: info?.detail?.title || info?.detail?.name || 'Tv',
      text: `Check out ${info?.detail?.title || info?.detail?.name || 'this tv'}`,
    ...
    <div className="fixed inset-0 bg-black/90 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="relative w-full max-w-6xl aspect-video bg-black rounded-2xl overflow-hidden shadow-2xl">
        ...
        <iframe
          ...
          title={trailerVideo.name || 'Tv Trailer'}
          frameBorder="0"
...
{info?.detail?.seasons && info.detail.seasons.length > 0 && (
  <div className="mb-8">
    ...
    <div className="overflow-x-auto tvScrollbar">
      <div className="flex gap-4 px-[5%] pb-4">
        {info.detail.seasons.map((season, index) => (
          <Link
            key={season.id || index}
            to={`/tvshows/details/${info.detail.id}/season/${season.season_number}`}
          ...
          >
            <div className="relative">
              <img
                ...
                src={`https://image.tmdb.org/t/p/w500${season.poster_path || season.backdrop_path}`}
                alt={season.name}
              />
              ...
              <div className="absolute bottom-0 left-0 right-0 p-3 text-white transform translate-y-2 group-hover:translate-y-0 transition-transform duration-200">
                <h3 className="font-semibold text-sm tvLineClamp2 mb-1">
                  {season.name}
                </h3>
                {season.air_date && (
              ...
              <div className="overflow-x-auto tvScrollbar">
                <div className="flex gap-4 px-[5%] pb-4">
                  {info.recommendations.map((tv, index) => (
                    <Link
                      key={tv.id || index}
                      to={`/${tv.media_type === "movie" ? "movies" : "tvshows"}/details/${tv.id}`}
                      className="flex-shrink-0 w-[200px] group cursor-pointer"
                    >
                      <div className="relative">
                        <img
                          className="w-full h-[280px] object-cover rounded-lg shadow-lg group-hover:scale-105 transition-transform duration-200"
                          src={`https://image.tmdb.org/t/p/w500${tv.poster_path || tv.backdrop_path}`}
       ...
       {/* Similar Tv Section */}
          {info?.similar && info.similar.length > 0 && (
            <div className="mb-8">
              <div className="px-[5%] mb-4">
                <h2 className="text-2xl font-bold text-white">Similar Tv</h2>
              </div>
              <div className="overflow-x-auto tvScrollbar">
                <div className="flex gap-4 px-[5%] pb-4">
                  {info.similar.map((tv, index) => (
                    <Link
                      key={tv.id || index}
                      to={`/tvshows/details/${tv.id}`} // Make sure your route matches this
                      className="flex-shrink-0 w-[200px] group cursor-pointer"
                    >
                      <div className="relative">
                        <img
